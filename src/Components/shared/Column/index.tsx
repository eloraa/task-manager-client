...
export const Column: React.FC<ColumnProps> = ({ item, index, refetch, updating }) => {
  ...
  return (
    <>
      ...
      {popup && (
        <TaskForm
          className="bg-coconut-faded text-black [&_input]:border-black/15 [&_textarea]:border-black/15 [&_select]:border-black/15 [&_textarea:focus]:border-black [&_select:focus]:border-black [&_input:focus]:border-black"
          refetch={refetch}
          task={item}
          setPopup={setPopup}
        ></TaskForm>
